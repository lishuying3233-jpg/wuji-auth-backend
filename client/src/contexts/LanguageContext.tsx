import React, { createContext, useContext, useEffect, useMemo, useState } from "react";

export type Locale = "zh-CN" | "en-US";

type TranslationParams = Record<string, string | number>;

type LanguageContextValue = {
  locale: Locale;
  languageName: string;
  setLocale: (locale: Locale) => void;
  toggleLocale: () => void;
  t: (key: string, params?: TranslationParams) => string;
};

const translations: Record<Locale, Record<string, string>> = {
  "zh-CN": {
    language: "语言",
    loading: "加载中",
    chinese: "中文",
    english: "English",
    brand: "M7社媒助手",
    finis: "完成",
    adminDashboard: "管理后台",
    loggedInAs: "当前账号：{name}",
    logout: "退出登录",
    cloudAuthorizationControl: "云端授权控制中心",
    prefix: "前缀",
    note: "备注",
    duration: "授权期限",
    batchCount: "数量",
    generate: "生成激活码",
    generating: "正在生成",
    generated: "成功生成 {count} 个激活码",
    generationFailed: "生成失败，请稍后重试",
    invalidPrefix: "前缀只能包含字母、数字和连字符，且不超过 24 个字符",
    invalidDuration: "授权期限无效，请重新选择",
    invalidCount: "生成数量必须是 1 到 50 之间的整数",
    totalLicenses: "授权码总数",
    activeDevices: "已激活设备",
    expired: "已过期",
    available: "待使用",
    licenses: "授权码管理",
    administrators: "管理员管理",
    all: "全部",
    active: "有效",
    unused: "未使用",
    search: "搜索激活码、机器码或备注",
    refresh: "刷新",
    licenseKey: "激活码",
    status: "状态",
    hwid: "机器码",
    expiry: "到期时间",
    actions: "操作",
    disabled: "已禁用",
    activeStatus: "已激活",
    unusedStatus: "未使用",
    pending: "未绑定",
    noExpiry: "不限期",
    noRecords: "暂无匹配记录",
    copy: "复制激活码",
    copied: "激活码已复制",
    statusUpdated: "状态更新成功",
    deleted: "删除成功",
    renew: "续期",
    renewLicense: "续期授权码",
    renewDescription: "为授权码 {code} 延长有效期",
    days: "天",
    renewSuccess: "续期成功",
    disable: "禁用授权码",
    enable: "启用授权码",
    delete: "删除",
    deleteConfirm: "确定要删除这个授权码吗？此操作无法撤销。",
    adminUser: "管理员账号",
    role: "角色",
    permissions: "权限",
    newAdmin: "新建管理员",
    username: "账号名称",
    password: "登录密码",
    superAdmin: "主管理员",
    subAdmin: "子管理员",
    createAdmin: "创建管理员",
    adminCreated: "子管理员创建成功",
    adminDeleted: "管理员已删除",
    createAdminHint: "账号用于登录后台，请妥善保存密码。",
    permissionGenerate: "生成激活码",
    permissionDelete: "删除激活码",
    permissionRenew: "续期授权码",
    permissionManageAdmins: "管理子管理员",
    secureFooter: "安全、轻盈的云端授权系统",
    secureProtocol: "安全协议 v0.1.8",
    homeTitle: "云端授权管理系统",
    homeSubtitle: "为 M7社媒助手提供安全、清晰、可控的授权管理",
    homeQuote: "在一个安静的控制台中，管理每一份软件授权。",
    adminLogin: "管理员登录",
    enterConsole: "进入控制台",
    welcome: "欢迎，{name}",
    loginTitle: "后台登录",
    loginSubtitle: "M7社媒助手云端授权管理",
    authenticate: "登录管理后台",
    loginSuccess: "登录成功",
    loginFailed: "登录失败",
    enterUsername: "请输入管理员账号",
    enterPassword: "请输入登录密码",
    authorizedPersonnel: "仅限授权人员使用",
    buildVersion: "构建版本: {version}",
    oneDay: "1 天",
    threeDays: "3 天",
    sevenDays: "7 天",
    oneMonth: "1 个月",
    threeMonths: "3 个月",
    oneYear: "1 年",
    renewSevenDays: "+7 天",
    renewOneMonth: "+30 天",
    renewOneYear: "+365 天",
    selectLanguage: "选择语言",
    selectAll: "全选当前列表",
    clearSelection: "清空选择",
    selectedCount: "已选择 {count} 项",
    batchDelete: "批量删除",
    batchDeleteConfirm: "确定删除已选择的 {count} 个激活码吗？此操作无法撤销。",
    batchDeleteLimit: "一次最多选择 1000 个激活码",
    batchDeleteSuccess: "已删除 {count} 个激活码",
    batchDeleteFailed: "批量删除失败，请稍后重试",
    selectLicense: "选择激活码 {code}",
    paymentManagement: "支付设置",
    orderManagement: "订单审核",
    network: "网络",
    address: "收款地址",
    addAddress: "添加地址",
    plan: "套餐",
    amount: "金额",
    txHash: "交易哈希",
    approve: "通过并结算",
    reject: "拒绝",
    orderTime: "下单时间",
    paymentConfirmed: "支付已确认，激活码已发放",
    orderRejected: "订单已拒绝",
    experienceCard: "体验卡",
    monthlyCard: "月卡",
    yearlyCard: "年卡",
    orderStatus: "订单状态",
    pendingAudit: "自动核验中",
    paidAudit: "待发码",
    completedAudit: "已完成",
    failedAudit: "已失败",
    notificationSettings: "通知设置",
    tgBotToken: "Telegram 机器人 Token",
    tgChatId: "Telegram 用户/群组 ID",
    tgEnabled: "启用通知推送",
    tgTest: "发送测试消息",
    tgTestSuccess: "测试消息已发出，请在 Telegram 中查收",
    tgTestFailed: "发送失败，请检查配置或网络",
    tgSaveSuccess: "通知设置已保存",
    tgSaveFailed: "保存失败，请稍后重试",
    tgSettingsHint: "主管理员可配置 Telegram 机器人，实时接收订单、激活和异常提醒。",
  },
  "en-US": {
    language: "Language",
    loading: "Loading",
    chinese: "中文",
    english: "English",
    brand: "M7 Social Media",
    finis: "Finis",
    adminDashboard: "Admin Dashboard",
    loggedInAs: "Logged in as {name}",
    logout: "Log out",
    cloudAuthorizationControl: "Cloud Authorization Control",
    prefix: "Prefix",
    note: "Note",
    duration: "License duration",
    batchCount: "Quantity",
    generate: "Generate licenses",
    generating: "Generating",
    generated: "Generated {count} license(s)",
    generationFailed: "Generation failed. Please try again.",
    invalidPrefix: "Prefix must use letters, numbers, or hyphens and be 24 characters or fewer.",
    invalidDuration: "Invalid license duration. Please choose again.",
    invalidCount: "Quantity must be an integer from 1 to 50.",
    totalLicenses: "Total licenses",
    activeDevices: "Active devices",
    expired: "Expired",
    available: "Available",
    licenses: "License management",
    administrators: "Administrators",
    all: "All",
    active: "Active",
    unused: "Unused",
    search: "Search license, machine ID, or note",
    refresh: "Refresh",
    licenseKey: "License key",
    status: "Status",
    hwid: "Machine ID",
    expiry: "Expiry",
    actions: "Actions",
    disabled: "Disabled",
    activeStatus: "Active",
    unusedStatus: "Unused",
    pending: "Pending",
    noExpiry: "No expiry",
    noRecords: "No matching records",
    copy: "Copy license",
    copied: "License copied",
    statusUpdated: "Status updated",
    deleted: "Deleted successfully",
    renew: "Renew",
    renewLicense: "Renew license",
    renewDescription: "Extend authorization for {code}",
    days: "days",
    renewSuccess: "Renewed successfully",
    disable: "Disable license",
    enable: "Enable license",
    delete: "Delete",
    deleteConfirm: "Delete this license? This action cannot be undone.",
    adminUser: "Admin account",
    role: "Role",
    permissions: "Permissions",
    newAdmin: "New administrator",
    username: "Username",
    password: "Password",
    superAdmin: "Super admin",
    subAdmin: "Sub admin",
    createAdmin: "Create administrator",
    adminCreated: "Sub-admin created",
    adminDeleted: "Administrator deleted",
    createAdminHint: "This account will be used to access the console. Store the password securely.",
    permissionGenerate: "Generate licenses",
    permissionDelete: "Delete licenses",
    permissionRenew: "Renew licenses",
    permissionManageAdmins: "Manage sub-admins",
    secureFooter: "A secure, light cloud authorization system",
    secureProtocol: "Secure protocol v0.1.8",
    homeTitle: "Cloud Authorization Management",
    homeSubtitle: "Secure, clear, and controlled authorization for M7 Social Media Assistant", 
    homeQuote: "Manage every software authorization from one quiet console.",
    adminLogin: "Administrator login",
    enterConsole: "Open console",
    welcome: "Welcome, {name}",
    loginTitle: "Console login",
    loginSubtitle: "M7 Social Media Assistant cloud authorization",
    authenticate: "Sign in to console",
    loginSuccess: "Signed in successfully",
    loginFailed: "Sign-in failed",
    enterUsername: "Enter administrator username",
    enterPassword: "Enter password",
    authorizedPersonnel: "Authorized personnel only",
    buildVersion: "Build Version: {version}",
    oneDay: "1 day",
    threeDays: "3 days",
    sevenDays: "7 days",
    oneMonth: "1 month",
    threeMonths: "3 months",
    oneYear: "1 year",
    renewSevenDays: "+7 days",
    renewOneMonth: "+30 days",
    renewOneYear: "+365 days",
    selectLanguage: "Select language",
    selectAll: "Select current list",
    clearSelection: "Clear selection",
    selectedCount: "{count} selected",
    batchDelete: "Delete selected",
    batchDeleteConfirm: "Delete the selected {count} license(s)? This action cannot be undone.",
    batchDeleteLimit: "Select up to 1,000 licenses at a time",
    batchDeleteSuccess: "Deleted {count} license(s)",
    batchDeleteFailed: "Batch delete failed. Please try again.",
    selectLicense: "Select license {code}",
    paymentManagement: "Payments",
    orderManagement: "Orders",
    network: "Network",
    address: "Address",
    addAddress: "Add Address",
    plan: "Plan",
    amount: "Amount",
    txHash: "TxHash",
    approve: "Approve & Issue",
    reject: "Reject",
    orderTime: "Order Time",
    paymentConfirmed: "Payment confirmed, license issued",
    orderRejected: "Order rejected",
    experienceCard: "Experience",
    monthlyCard: "Monthly",
    yearlyCard: "Yearly",
    orderStatus: "Status",
    pendingAudit: "Verifying",
    paidAudit: "To Issue",
    completedAudit: "Completed",
    failedAudit: "Failed",
    notificationSettings: "Notifications",
    tgBotToken: "Telegram Bot Token",
    tgChatId: "Telegram Chat ID",
    tgEnabled: "Enable Push Notifications",
    tgTest: "Send Test Message",
    tgTestSuccess: "Test message sent, please check Telegram",
    tgTestFailed: "Failed to send, check config or network",
    tgSaveSuccess: "Settings saved",
    tgSaveFailed: "Failed to save, try again later",
    tgSettingsHint: "Super admin can configure Telegram bot to receive real-time alerts.",
  },
};

const LanguageContext = createContext<LanguageContextValue | undefined>(undefined);

function interpolate(value: string, params: TranslationParams = {}) {
  return value.replace(/\{(\w+)\}/g, (_, key: string) => String(params[key] ?? `{${key}}`));
}

export function LanguageProvider({ children, defaultLocale = "zh-CN" }: { children: React.ReactNode; defaultLocale?: Locale }) {
  const [locale, setLocaleState] = useState<Locale>(() => {
    if (typeof window === "undefined") return defaultLocale;
    const stored = window.localStorage.getItem("wuji-admin-locale");
    return stored === "en-US" || stored === "zh-CN" ? stored : defaultLocale;
  });

  useEffect(() => {
    window.localStorage.setItem("wuji-admin-locale", locale);
    document.documentElement.lang = locale;
  }, [locale]);

  const value = useMemo<LanguageContextValue>(() => ({
    locale,
    languageName: locale === "zh-CN" ? "中文" : "English",
    setLocale: setLocaleState,
    toggleLocale: () => setLocaleState(current => current === "zh-CN" ? "en-US" : "zh-CN"),
    t: (key, params) => interpolate(translations[locale][key] ?? translations["zh-CN"][key] ?? key, params),
  }), [locale]);

  return <LanguageContext.Provider value={value}>{children}</LanguageContext.Provider>;
}

export function useLanguage() {
  const context = useContext(LanguageContext);
  if (!context) throw new Error("useLanguage must be used within LanguageProvider");
  return context;
}

export const languageMessages = translations;
